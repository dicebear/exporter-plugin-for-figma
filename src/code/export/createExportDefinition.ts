...
export async function createExportDefinition(exportData: Export) {
  ...
  for (const [colorGroupKey, colorGroupValue] of Object.entries(exportData.colors)) {
    ...
    colors[colorGroupKey] = {
      notEqualWith: Object.entries(colorGroupValue.settings.notEqualWith ?? {})
        .filter(([key, value]) => value && exportData.colors[key]?.isUsedByComponents)
        .map(([key]) => key),
  ...
  return JSON.stringify(
    removeEmptyValuesFromObject({
      $schema: 'https://www.dicebear.com/schemas/9.x/definition.json',
